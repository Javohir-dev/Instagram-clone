...
class User(AbstractUser, BaseModel):
    USER_ROLES = ((ORDINARY_USER, ORDINARY_USER), (MANAGER, MANAGER), (ADMIN, ADMIN))
    AUTH_TYPE_CHOICES = ((VIA_PHONE_NUMBER, VIA_PHONE_NUMBER), (VIA_EMAIL, VIA_EMAIL))
    AUTH_STATUS = (
        (NEW, NEW),
    ...
    def check_email(self):
        if self.email:
            normalize_email = (
                self.email.lower()
            )  # aKhamdjon@gmail.com -> akhamdjon@gmail.com
            self.email = normalize_email

    def check_pass(self):
        if not self.password:
            temp_password = (
                f'password-{uuid.uuid4().__str__().split("-")[-1]}'  #  123456mfdsjfkd
            )
